...
if ($part != ""){
	...
	$p = "update `Grades` SET `part` = '$part' where user = '$user' and course = '$course' ";

	($t3 = mysql_query($p)) or die (mysql_error());

}



mysql_query("UPDATE `Grades` SET `total`=`a1`+`a2`+`part`

            WHERE user = '$user' and course = '$course'");

mysql_query("UPDATE `Grades` SET Percent=100*total/150
